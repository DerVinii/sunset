...
import { ArrowIcon, BadgeDark, Container, CtaButton, H2, Lead, Section } from "@/components/ui-basics";
...
import { SunsetHero } from "@/components/hero-3d/sunset-hero";
      ...
      <SunsetHero>
        <Container className="py-24 sm:py-32 lg:py-40">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            <BadgeDark>Angebot bis zum nächsten Werktag, 12:00 Uhr</BadgeDark>
            <BadgeDark>Festpreis im Angebot — keine Nachberechnungen</BadgeDark>
            <BadgeDark>Alles aus einer Hand</BadgeDark>
          </div>
          <div className="mt-12 grid gap-4 sm:grid-cols-3">
            {einstiege.map((e) => (
              <Link
                key={e.href}
                href={e.href}
                className="group rounded-2xl border border-white/15 bg-white/8 p-5 backdrop-blur transition-all duration-200 hover:-translate-y-1 hover:border-amber-400/40 hover:bg-white/12"
              >
                <p className="flex items-center justify-between font-semibold text-white">
                  {e.title}
                  <ArrowIcon className="h-4 w-4 text-amber-400 transition-transform duration-200 group-hover:translate-x-1" />
                </p>
                <p className="mt-1.5 text-sm leading-relaxed text-stone-300">{e.text}</p>
              </Link>
            ))}
          </div>
        </Container>
