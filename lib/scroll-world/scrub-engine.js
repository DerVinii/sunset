   ...
   the rest of the phone handling below is always on)
     The engine is phone-aware out of the box: on a coarse-pointer / ≤860px viewport it
       ...
       - keeps the still as a live poster until the clip actually paints its first frame,
         and primes each video (muted play→pause) THE MOMENT it is attached — never later.
         iOS ignores `preload` and only fetches media data once play() has been called, so
         priming from a 'loadeddata'/'canplay' handler deadlocks: the event waits for data
         the browser will not fetch until you prime. Any scene attached after that point
         would sit on its poster forever. Every touch re-primes whatever still has no data.
       - separates download from decoding: the Blob is kept (plain memory, ~2 MB a clip)
         while only the scenes around the current one hold a <video> element. iOS grants a
         page just a handful of simultaneous video decoders; beyond that, further scenes
         silently never decode. Re-attaching is instant because the Blob is still there.
...
function mountScrollWorld(container, config) {
  ...
  // Alle Ebenen leben in einer gepinnten Bühne (position:sticky), nicht global fixed.
  // Dadurch klebt die Welt nur, solange ihr eigener Abschnitt läuft, und gibt den Rest
  // der Seite danach frei — Pflicht, sobald unter der Welt noch Inhalt folgt.
  const pin = el('div', 'sw-pin');
  [sky, scrollbar, topbar, stage, copylayer, route, hint].forEach(n => {
    if (n === topbar && !topbar.childElementCount) return;      // kein leerer Balken
    if (n === scrollbar && config.progress === false) return;   // Fortschritt über die Punkte
    pin.appendChild(n);
  });
  ...
  // Dokument-Y, an dem die Welt beginnt. Alle Segment-Positionen sind relativ dazu, damit
  // über der Welt Inhalt stehen darf (z. B. ein Seiten-Header) ohne den Flug zu verschieben.
  let originY = 0;
  const measureOrigin = () => container.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);

  function layout() {
    vh = window.innerHeight;
    laidOutW = window.innerWidth;
    originY = measureOrigin();
    stageX = window.innerWidth > 860 ? 4 : 0;
    let off = 0;
    SEGMENTS.forEach(s => { s.start = off * vh; off += s.w; s.end = off * vh; });
    totalW = off;
    // Die gepinnte Bühne selbst liefert die letzte Viewport-Höhe Auslauf: die Welt klebt
    // exakt so lange, wie der Track hoch ist, und löst sich danach nach oben ab.
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: originY + seg.start + (seg.end - seg.start) * 0.5, behavior: reduce ? 'auto' : 'smooth' });
  }

  // ---- Ladesteuerung ----------------------------------------------------------
  // Ein Clip wird komplett heruntergeladen, BEVOR er das erste Bild zeigen kann. Wenn
  // alle Clips im Umkreis gleichzeitig laden, teilen sie sich die Bandbreite und
  // ausgerechnet der als Nächstes gebrauchte bekommt nur einen Bruchteil — auf dem
  // Handy kommt er dann zu spät und man sieht bis zum harten Schnitt nur das Standbild.
  // Deshalb: eine Warteschlange, die immer den nächstgelegenen Clip zuerst und mit
  // voller Bandbreite lädt (auf dem Handy einzeln, am Desktop zu dritt).
  let inflight = 0;
  const loadQueue = [];
  const maxInflight = () => (isMobile() ? 1 : 3);
  // Wie viele Szenen rund um die aktuelle ein echtes <video>-Element bekommen.
  const attachRadius = () => (isMobile() ? 1 : NSEG);

  function requestClip(s, dist) {
    if (reduce || s.loading || s.blob || !s.clip) return;
    s.dist = dist;
    if (!s.queued) { s.queued = true; loadQueue.push(s); }
  }

  function pump() {
    while (inflight < maxInflight() && loadQueue.length) {
      loadQueue.sort((a, b) => a.dist - b.dist);
  ...
  // Schritt 1 — nur die Daten holen. Ein Blob ist reiner Speicher (alle neun zusammen
  // ~15 MB) und belegt noch keinen Video-Dekoder.
  function loadClip(s) {
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => { s.blob = blob; read(); })
      .catch(() => {})
      .then(() => { s.loading = false; inflight--; pump(); });
  }

  // Schritt 2 — Video-Element anhängen. Erst das belegt einen Dekoder, deshalb bekommen
  // auf dem Handy nur die Szenen rund um die aktuelle eines.
  function attachVideo(s) {
    if (reduce || s.hasClip || !s.blob) return;
    ...
    s.seekStart = 0; s.waitStart = 0;
    if (s.retries == null) s.retries = 0;
    s.cur = s.target;   // beim Anhängen sofort an die richtige Stelle, nicht erst hinfahren
    // WICHTIG (das war der Fehler): iOS ignoriert `preload` und holt die Videodaten erst,
    // wenn play() gerufen wurde. Stößt man play() erst im Ereignis 'loadeddata' an, wartet
    // man ewig — dieses Ereignis setzt die Daten ja voraus. Genau daran blieb jede Szene
    // hängen, deren Element erst nach der ersten Berührung entstand: sie zeigte dauerhaft
    // ihr Standbild. Also sofort beim Anhängen anstoßen.
    try { v.load(); } catch (e) {}
    primeVideo(v);
  }

  // Element und damit den Dekoder freigeben. Die geladenen Daten bleiben liegen, damit
  // erneutes Anhängen ohne Nachladen sofort geht.
  ...
  function read() {
    originY = measureOrigin();
    const y = (window.scrollY || window.pageYOffset) - originY;
    ...
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      // Abstand zum aktuellen Blickpunkt (0 = wir sind mittendrin) — Reihenfolge im Ladeplan.
      const gap = (y < s.start) ? (s.start - y) : (y > s.end ? y - s.end : 0);
      if (gap < 2.5 * vh) requestClip(s, gap);
      const local = clamp((y - s.start) / (s.end - s.start), 0, 1);
      s.target = s.linger ? lingerEase(local, s.linger) : local;
      // Dekoder-Budget: am Desktop alle, auf dem Handy nur die Szene ringsum. iOS gibt
      // einer Seite nur eine Handvoll gleichzeitiger Video-Dekoder; sind sie belegt,
      // bleiben weitere Szenen einfach auf ihrem Standbild stehen.
      if (Math.abs(i - ci) <= attachRadius()) attachVideo(s); else detachVideo(s);
      // Die äußeren Ränder der Kette blenden NICHT aus: das erste Bild steht beim Landen
      // sofort, das letzte bleibt stehen, während die Bühne nach oben abfährt. Sonst
      // liefe man am Ende gegen einen leeren Himmel.
      let outside = 0;
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      c.style.setProperty('--sw-shift', reduce ? '0vh' : `${((0.5 - pr) * 4).toFixed(2)}vh`);
    ...
    // Ablösung: sobald der Flug zu Ende ist, fährt die Bühne nach oben aus dem Bild.
    // Text und Wegweiser blenden dabei weg und die Unterkante löst sich ins Seitenweiß
    // auf — sonst würde die Welt sichtbar "abgeschnitten" nach oben wegrutschen.
    pump();
    const exit = clamp((y - totalW * vh) / (0.5 * vh));
    pin.style.setProperty('--sw-exit', exit.toFixed(3));
    copylayer.style.opacity = String(1 - exit);
    route.style.opacity = String(1 - exit);
    scrollbar.style.opacity = String(1 - exit);
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (!s.hasClip || !s.video) continue;
      // Angehängt, aber es kommen keine Daten. Auf iOS heißt das: play() ging nicht durch
      // (z. B. Stromsparmodus). Ohne Gegenmaßnahme bliebe die Szene für immer auf ihrem
      // Standbild stehen — also nach kurzer Wartezeit erneut anstoßen.
      if (!s.ready) {
        if (!s.waitStart) s.waitStart = now();
        else if (now() - s.waitStart > 1500 && s.retries < 4) {
          s.retries++; s.waitStart = 0;
      ...
      if (s.video.seeking) {
        // Bleibt ein Seek unbeantwortet, hat iOS dem Element den Dekoder entzogen.
        // Dann hilft nur neu anhängen — die Daten liegen ja noch bereit.
        if (!s.seekStart) s.seekStart = now();
        else if (now() - s.seekStart > 2500 && s.retries < 4) {
          s.retries++; s.seekStart = 0;
          detachVideo(s); attachVideo(s);
        }
        continue;
      }
      s.seekStart = 0;
  ...
  // "Anstoßen" = ein kurzes stummes play()→pause(). Auf iOS ist das nicht bloß Kosmetik:
  // erst play() bringt das System dazu, die Videodaten überhaupt zu holen (preload wird
  // dort ignoriert), und erst dann malt es beim Scrubben ein Bild. Angestoßen wird
  // deshalb sofort beim Anhängen (siehe attachVideo) — und zusätzlich bei jeder
  // Berührung für alles, was noch keine Daten hat.
  ...
  // Nicht nur beim ERSTEN Tippen: jede Berührung zieht alles nach, was noch keine Daten
  // hat. Auf iOS beginnt jedes Wischen mit einem touchstart, also repariert sich eine
  // hängengebliebene Szene beim nächsten Scrollen von selbst.
  function primePending() {
    SEGMENTS.forEach(s => { if (s.video && !s.ready) { try { s.video.load(); } catch (e) {} primeVideo(s.video); } });
  }
  window.addEventListener('pointerdown', primePending, { passive: true });
  window.addEventListener('touchstart', primePending, { passive: true });
...
function injectCSS() {
  ...
  .sw-root{--sw-bg:#F5EDE0;--sw-ink:#241d2b;--sw-ink-soft:#6a6072;--sw-accent:#8a7bb5;
    ...
    position:relative;z-index:0;isolation:isolate;background:var(--sw-bg);}
  /* Gepinnte Bühne: klebt genau so lange, wie der Track hoch ist, und fährt danach
     mit dem Abschnitt nach oben ab. Alles darunter auf der Seite bleibt frei. */
  .sw-pin{position:sticky;top:0;height:100vh;height:100svh;overflow:hidden;background:var(--sw-bg);}
  .sw-pin::after{content:"";position:absolute;left:0;right:0;bottom:0;height:16vh;z-index:26;
    pointer-events:none;opacity:var(--sw-exit,0);background:linear-gradient(0deg,var(--sw-bg),transparent);}
  ...
  .sw-scrollbar{position:absolute;left:0;right:0;bottom:0;height:3px;z-index:60;background:color-mix(in srgb,var(--sw-accent) 14%,transparent);}
  .sw-scrollbar span{display:block;height:100%;width:100%;transform-origin:0 50%;transform:scaleX(0);background:var(--sw-accent);}
  .sw-topbar{position:absolute;top:0;left:0;right:0;z-index:50;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:clamp(14px,2.4vw,26px) clamp(18px,5vw,64px);}
  ...
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:50%;transform:translateY(calc(-50% + var(--sw-shift,0vh)));width:min(42vw,460px);opacity:0;will-change:opacity,transform;}
  ...
  @media (max-width:860px){
    ...
    .sw-copy{left:clamp(18px,5vw,64px);right:clamp(18px,5vw,64px);top:auto;bottom:clamp(64px,14vh,120px);transform:translateY(var(--sw-shift,0vh));width:auto;max-width:560px;}
...
// ESM-Export für Next.js/Turbopack (Engine bleibt ansonsten unverändert).
