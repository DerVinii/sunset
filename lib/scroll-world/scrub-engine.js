...
function mountScrollWorld(container, config) {
  ...
  // Weit entfernte Clips auf dem Handy wieder freigeben. iOS gibt einer Seite nur wenig
  // Speicher und nur eine begrenzte Zahl aktiver Video-Dekoder; hält man alle neun offen,
  // wirft das System irgendwann welche raus und sie bleiben schwarz. Das erneute Laden
  // kommt aus dem Browser-Cache und kostet daher praktisch nichts.
  function unloadClip(s) {
    if (!s.hasClip || s.visible) return;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      if (gap < 1.6 * vh) requestClip(s, gap);
      else if (isMobile() && gap > 3 * vh) unloadClip(s);
